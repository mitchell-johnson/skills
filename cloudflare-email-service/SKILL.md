...
**Current status:** Email Sending is in open beta and requires a Workers Paid plan; Email Routing is GA (free, unlimited).
...
- Response `result` contains `delivered` / `permanent_bounces` / `queued` arrays (the OpenAPI schema also includes `message_id`)

See [rest-api.md](references/rest-api.md) for curl examples, response format, and error handling.

## Pricing & Limits

- **Pricing:** Workers Paid includes 3,000 emails/month, then $0.35 per 1,000. Sends to verified destination addresses are free on all plans and don't count toward quota; rejected/suppressed sends don't consume quota either. Email Routing is free and unlimited. [Pricing docs](https://developers.cloudflare.com/email-service/platform/pricing/)
- **Limits:** 5 MiB max outbound message (25 MiB to verified destinations) · 32 attachments max · subject max 998 chars · 30 sending domains/zone · 200 routing rules/domain · 200 destination addresses/account · 25 MiB inbound. [Limits docs](https://developers.cloudflare.com/email-service/platform/limits/)
