...
**Important:** Attachment `content` handling:
- **Workers binding:** `content` accepts `string | ArrayBuffer | ArrayBufferView`. Per current docs, string content should be **base64-encoded**; binary content can be passed directly as `ArrayBuffer`/`ArrayBufferView` (no base64 needed). Docs/runtime ambiguity has been observed here — if a base64 string arrives corrupted, consult the [attachments docs](https://developers.cloudflare.com/email-service/api/send-emails/workers-api/#attachments) for current behavior. Note: `ArrayBuffer` does not work with `"remote": true` in local dev — deploy to test binary attachments.
- **REST API:** `content` is always a **base64-encoded string**. See [rest-api.md](rest-api.md).

```typescript
// Text file attachment — base64-encode string content
...
const response = await env.EMAIL.send({
  ...
  attachments: [{
    content: btoa(csv), // Base64-encoded string, per current docs
    filename: "report.csv",
    type: "text/csv",
...
Total message size (body + attachments) cannot exceed 5 MiB — 25 MiB only when sending to [verified destination addresses](https://developers.cloudflare.com/email-service/platform/limits/). Max 32 attachments per email.

## Custom Headers

Only whitelisted headers allowed — see the [headers reference](https://developers.cloudflare.com/email-service/reference/headers/). Limits: max 20 non-`X-` custom headers, 16 KB total headers, 2,048 bytes per value, 100 bytes per name. Never set `From`, `To`, `Reply-To`, etc. via `headers` — use the dedicated API fields.
...
| `E_TOO_MANY_ATTACHMENTS` | More than 32 attachments | Reduce attachment count |
...
| `E_CONTENT_TOO_LARGE` | Content exceeds 5 MiB (25 MiB to verified destinations) | Reduce attachments or body |
...
For `E_RATE_LIMIT_EXCEEDED`, `E_DELIVERY_FAILED`, and `E_INTERNAL_SERVER_ERROR`, retry with exponential backoff. For validation errors (`E_VALIDATION_ERROR`, `E_FIELD_MISSING`, `E_SENDER_NOT_VERIFIED`), fix the request — retrying won't help.

### Pattern: one app-level email helper

Wrap sends behind a single helper so logging, retry policy, and any future provider migration live in one place instead of at every call site:

```typescript
export async function sendEmail(env: Env, message: Parameters<Env["EMAIL"]["send"]>[0]) {
  try {
    return await env.EMAIL.send(message);
  } catch (error: any) {
    const retryable = ["E_RATE_LIMIT_EXCEEDED", "E_DELIVERY_FAILED", "E_INTERNAL_SERVER_ERROR"];
    console.error(`email send failed: ${error.code} - ${error.message}`);
    throw Object.assign(error, { retryable: retryable.includes(error.code) });
  }
}
```
