...
For the full OpenAPI specification, refer to the [Email Sending API reference](https://developers.cloudflare.com/api/resources/email_sending/methods/send). Official REST SDKs are available for Node.js, Python, and Go — linked from the [REST API docs](https://developers.cloudflare.com/email-service/api/send-emails/rest-api/).
...
For pre-built raw MIME messages, use `POST /accounts/{account_id}/email/sending/send_raw` instead.
...
The OpenAPI schema also includes a `message_id` field in the result.

Error — 5-digit numeric codes (10000–10203) with machine-readable messages, not `E_*` string codes like Workers:
...
Other examples: `10004 email.sending.error.throttled`. See the [error handling reference](https://developers.cloudflare.com/email-service/api/send-emails/rest-api/#error-handling) for the full code table.
...
Email Service also accepts SMTP submission — useful for apps and tools that only speak SMTP:

- Host: `smtp.mx.cloudflare.net`, port `465` with implicit TLS only (no port 587 / STARTTLS)
- Auth: `AUTH PLAIN` or `AUTH LOGIN` — username is literally `api_token`, password is a Cloudflare API token with **Email Sending: Edit** permission

See the [SMTP docs](https://developers.cloudflare.com/email-service/api/send-emails/smtp/).
