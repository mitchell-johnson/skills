...
Set up routing rules in **Dashboard** > **Compute** > **Email Service** > **Email Routing** > **Routing Rules**, or via `wrangler email routing rules create`.
...
- `message.canBeForwarded` — whether this message is eligible to be forwarded
...
Only `X-`-prefixed extra headers are accepted — any other header you pass is stripped before the message is forwarded.
...
`message.reply()` enforces several constraints — violating any of them throws:

- The incoming message must **pass DMARC**.
- Only **one reply per email event**.
- The reply recipient must be the **original sender** (`message.from`).
- The outgoing sender domain must **match the receiving domain**.
- The `References` header may contain at most **100 entries**.
...
- **"Dropped" in the Email Routing summary is misleading.** Emails sent from a Worker show up as `dropped` in the Email Routing summary even when they were delivered successfully. Use the Email Sending analytics for real delivery status.
