...
├─ On-demand isolated Workers for agent code execution → Dynamic Workers (see Product Index)
...
├─ Streaming ingestion to R2 (Parquet/JSON) or Apache Iceberg tables in R2 Data Catalog → pipelines/ (open beta, Workers Paid)
...
├─ Managed RAG pipeline / agentic search (formerly AutoRAG; GA on all plans) → ai-search/
└─ Managed persistent memory for agents → Agent Memory (see Product Index)
```
...
├─ Browser automation/screenshots → browser-run/ (formerly Browser Rendering)
...
| Dynamic Workers | https://developers.cloudflare.com/dynamic-workers/ (on-demand isolated Workers for agent code execution; no local reference) |
...
| Agent Memory | https://developers.cloudflare.com/agent-memory/ (private beta; managed persistent memory for agents; no local reference) |
...
| Browser Run (formerly Browser Rendering) | `references/browser-run/` |
...
| Email Service | https://developers.cloudflare.com/email-service/ (Email Sending + Email Routing unified; outbound transactional email in beta — the dedicated sibling skill `cloudflare-email-service` covers it in depth) |
