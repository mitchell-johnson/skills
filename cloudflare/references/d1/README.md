...
```typescript
// Sequential consistency across queries when read replication is enabled
// Docs: https://developers.cloudflare.com/d1/best-practices/read-replication/
const session = env.DB.withSession("first-primary");  // first query hits primary (read-your-writes)
// or: env.DB.withSession("first-unconstrained")      // first query can hit any replica
// or: env.DB.withSession(bookmark)                   // resume from a prior session's bookmark

const user = await session.prepare('SELECT * FROM users WHERE id = ?').bind(userId).first();
await session.prepare('UPDATE users SET last_login = ? WHERE id = ?').bind(Date.now(), userId).run();

// Pass the bookmark to the client (header/cookie) for continuity across requests
const bookmark = session.getBookmark();
```

## Read Replication

Enabled per-database; reads route to the nearest replica through the Sessions API on the same `env.DB` binding (there is no separate replica binding). Writes always go to the primary. Built into D1 at no extra storage or compute cost. See https://developers.cloudflare.com/d1/best-practices/read-replication/
...
| Row/string/BLOB size | 2,000,000 bytes (2 MB) max | 2,000,000 bytes (2 MB) max |
