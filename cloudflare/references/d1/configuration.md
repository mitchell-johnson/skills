...
```jsonc
{
  ...
  "d1_databases": [
    ...
    // Read replication needs no extra binding - enable it per database in the
    // dashboard or via the REST API, then read through env.DB.withSession(...)
...
| Read replication | ✅ | ✅ (no extra storage or compute cost) |
| Sessions API | ✅ | ✅ |
