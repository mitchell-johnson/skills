...
Configured per bucket via the dashboard or the CLI - there is no `event_notifications` key in wrangler config.

```bash
wrangler r2 bucket notification create my-bucket --event-type object-create --queue r2-events
wrangler r2 bucket notification create my-bucket --event-type object-delete --queue r2-events --prefix uploads/
wrangler r2 bucket notification delete my-bucket --queue r2-events
```

The Worker that consumes the events declares the queue in wrangler.jsonc (R2 itself is the producer):
