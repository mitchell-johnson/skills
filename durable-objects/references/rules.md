...
Influence DO creation location for latency-sensitive apps. The hint is an option on `get()` (and `getByName()`), not `idFromName()`, and only the first `get()` for an object respects it:

```typescript
const id = env.GAME.idFromName(gameId);
const stub = env.GAME.get(id, { locationHint: "wnam" });
// or: env.GAME.getByName(gameId, { locationHint: "wnam" })
```

Available hints: `wnam`, `enam`, `sam`, `weur`, `eeur`, `apac`, `apac-ne`, `apac-se`, `oc`, `afr`, `me`.

## Storage

### SQLite (Default)

SQLite is the default and only backend for most new namespaces — KV-backed namespaces are no longer available for new classes. Declare in wrangler:
```jsonc
...
The SQLite backend also exposes a synchronous KV API (alongside the async `ctx.storage.get/put`):
```typescript
...
**Note:** `PRAGMA user_version` is not documented as supported in Durable Objects SQLite storage. Use a `_sql_schema_migrations` table instead:
...
SQLite-backed storage keeps 30 days of history. Use bookmarks to restore:

```typescript
const current = await this.ctx.storage.getCurrentBookmark();
const older = await this.ctx.storage.getBookmarkForTime(Date.now() - 3600_000);
await this.ctx.storage.onNextSessionRestoreBookmark(older); // applies on next restart
```
...
**Retry**: Failed alarms retry up to 6 times with exponential backoff starting at 2 seconds. The handler receives an optional argument — `alarm(alarmInfo)` with `alarmInfo.retryCount` and `alarmInfo.isRetry`. Keep handlers idempotent.
...
// Attach per-socket state that survives hibernation (16 KB max)
ws.serializeAttachment({ userId });
const { userId } = ws.deserializeAttachment();

// Reply to pings without waking the hibernated DO
