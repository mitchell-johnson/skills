...
## Integration Tests (Worker Entrypoint)

Call the Worker's default export via `exports` from `cloudflare:workers`:
...
describe("Worker HTTP", () => {
  it("should increment via POST", async () => {
    ...
    const res = await exports.default.fetch(
      new Request("http://example.com?id=test", { method: "POST" }),
      env,
      ctx
    );
  ...
  it("should get count via GET", async () => {
    const ctx = createExecutionContext();
    const post = () => new Request("http://example.com?id=get-test", { method: "POST" });
    await exports.default.fetch(post(), env, ctx);
    await exports.default.fetch(post(), env, ctx);

    const res = await exports.default.fetch(
      new Request("http://example.com?id=get-test"),
      env,
      ctx
    );
    await waitOnExecutionContext(ctx);
...
`SELF` from `cloudflare:test` (`SELF.fetch(...)`) is the legacy equivalent.
...
Use `evictDurableObject()` to simulate eviction — in-memory state is dropped, storage persists:

```typescript
import { env } from "cloudflare:workers";
import { evictDurableObject } from "cloudflare:test";

const stub = env.COUNTER.getByName("evict-test");
await stub.increment();
await evictDurableObject(stub);
// Next access constructs a fresh instance; SQLite data survives
```
